...
const float kp[4] = {4, 4, 10, 8};                // Mathieu: {4, 5, 10, 8}
const float ki[4] = {0, 0, 0.5, 0.5};         // Mathieu: {0.1, 0.1, 0.5, 0.5}
const float kd[4] = {0, 0, 10, 10};               // Mathieu: {3, 3, 10, 10}

const float ff[2][6] = {{0.0004, 0.0014, 0.6, 0.55, 0.2, 0.44},  // Open jaw ff values, hTop, hBot, offsetTop, offsetBot, difTop, difBot 
                        {0, 0, 0, 0, 0, 0}}; // Closed jaw feed forward variables, hBot was 0.0018 {0.0012, 0.0021, 0.75, 0.65, 0.48, 0.5}
...
void logTempCSV(int plot, bool logPID)
{
  /*
...
void readButtons()
{
  if (digitalRead(HOT_ENABLE) && heatEnable)
  { // if heat is turned off but was on
    heatEnable = 0;
  ...
  else if (!digitalRead(HOT_ENABLE) && !heatEnable)
  { // if heat is turned on but was off (also activates on startup)
    heatEnable = 1;
...
void runProgram()
{
  if (startProgram && !programStarted) // if the program is called to start, but was not yet started
  {
    ...
    programStarted = 1;     // program has started
